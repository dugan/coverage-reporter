...
def filter_paths(path_list, covered_paths, path_filter=None):
    if not path_list:
        # No paths were specified by the user, so we only report on covered files.
        path_list = covered_paths
        for path in path_list:
            if path_filter and path_filter.filter(path):
                yield path
    else:
        for x in filter_specified_paths(path_list, covered_paths, path_filter):
            yield x

def filter_specified_paths(path_list, covered_paths, path_filter=None):
    """
        ...
        if os.path.isdir(specified_path):
            for root, dirs, filenames in os.walk(specified_path):
                for filename in filenames:
                    ...
                    if path_filter and path_filter.filter(full_path):
                        yield full_path        
        else:
